...
async def obtener_plato(plato_id: UUID, db: Session = Depends(get_db)):
    try:
        ...
        if not plato:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Plato no encontrado"
            )
        return plato
...
async def actualizar_plato(
    plato_id: UUID, datos_plato: PlatoUpdate, db: Session = Depends(get_db)
):
    try:
        plato_crud = PlatoCRUD(db)
        ...
        plato_actualizado = plato_crud.actualizar_plato(
            plato_id, id_usuario_mod=id_usuario_mod, **datos_actualizar
        )
...
async def eliminar_plato(plato_id: UUID, db: Session = Depends(get_db)):
    try:
        ...
        if eliminado:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Error al eliminar plato",
            )
    except HTTPException:
        raise
    except Exception as e:
